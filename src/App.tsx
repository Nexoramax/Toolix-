...
import { 
  ...
  RefreshCw, Clock, Server, Cpu, HelpCircle
} from 'lucide-react';
...
export default function App() {
  const getInitialRoute = (): { tab: any; toolId: string | null; blogPostId: string | null } => {
    const path = window.location.pathname;
    if (path === '/' || path === '') {
      return { tab: 'home', toolId: null, blogPostId: null };
    }
    const cleanPath = path.replace(/^\/+|\/+$/g, '');
    if (['about', 'privacy', 'terms', 'contact', 'tools', 'favorites', 'history', 'settings', 'status', 'cookie', 'disclaimer', 'mission', 'vision', 'team', 'story', 'support', 'blog', 'sitemap'].includes(cleanPath)) {
      return { tab: cleanPath as any, toolId: null, blogPostId: null };
    }
    if (cleanPath.startsWith('tool/')) {
      const toolId = cleanPath.substring(5);
      return { tab: 'tools', toolId: toolId, blogPostId: null };
    }
    if (cleanPath.startsWith('blog/')) {
      const blogPostId = cleanPath.substring(5);
      return { tab: 'blog-post', toolId: null, blogPostId: blogPostId };
    }
    return { tab: '404' as any, toolId: null, blogPostId: null };
  };

  const initialRoute = getInitialRoute();
  const [language, setLanguage] = useState<'en' | 'ar'>('en');
  const [darkMode, setDarkMode] = useState<boolean>(true);
  const [activeTab, setActiveTab] = useState<'home' | 'tools' | 'favorites' | 'history' | 'settings' | 'about' | 'privacy' | 'terms' | 'cookie' | 'disclaimer' | 'contact' | '404' | 'status' | 'mission' | 'vision' | 'team' | 'story' | 'support' | 'blog' | 'blog-post' | 'sitemap'>(initialRoute.tab);
  ...
  const [selectedBlogPostId, setSelectedBlogPostId] = useState<string | null>(initialRoute.blogPostId);

  const navigateTo = (
    tab: 'home' | 'tools' | 'favorites' | 'history' | 'settings' | 'about' | 'privacy' | 'terms' | 'cookie' | 'disclaimer' | 'contact' | '404' | 'status' | 'mission' | 'vision' | 'team' | 'story' | 'support' | 'blog' | 'blog-post' | 'sitemap', 
  ...
  useEffect(() => {
    ...
    if (selectedToolId && activeTool) {
      ...
      if (activeTab === 'about') {
        ...
        title = language === 'en'
          ? 'Cookie Policy | Complete Cookies & Ads Disclosure - Toolix'
          : 'سياسة ملفات تعريف الارتباط والشفافية الإعلانية - توليكس';
        description = language === 'en'
          ? 'Our Cookie Policy outlines how we utilize cookies, web analytics, and Google AdSense partner technologies to support our free client-side services.'
          : 'سياسة ملفات تعريف الارتباط في توليكس. تعرف على كيفية استخدام ملفات الكوكيز وإعلانات غوغل أدسنس والتحليلات لدعم خدماتنا البرمجية المجانية.';
      } else if (activeTab === 'disclaimer') {
        title = language === 'en'
          ? 'Disclaimer & Accuracy Disclosures - Toolix'
          : 'إخلاء المسؤولية القانونية وحدود الضمان - توليكس';
        description = language === 'en'
          ? 'Review our legal disclaimers. Toolix is a browser sandbox offered on an as-is basis, providing private client-side calculators, compressors, and helpers.'
          : 'إخلاء المسؤولية القانونية لمنصة توليكس. تشغيل محلي 100% للأدوات الحسابية والهندسية المتوفرة "كما هي" دون أي ادعاء بالضمان أو المسؤولية.';
    ...
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-[#030712] text-slate-100' : 'bg-[#F8FAFC] text-slate-900'}`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <header className={`sticky top-0 z-40 border-b backdrop-blur-lg transition-colors duration-300 ${darkMode ? 'bg-[#030712]/80 border-slate-900' : 'bg-white/80 border-slate-200'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-[72px] flex items-center justify-between">
          <div className="flex items-center gap-1.5 cursor-pointer overflow-visible" onClick={() => navigateTo('home')}>
            <img 
              src="/favicon.ico" 
              onError={(e) => {
                const img = e.currentTarget;
                if (img.src.endsWith('/favicon.ico')) {
                  img.src = '/android-chrome-1024x1024.png';
                } else {
                  img.style.display = 'none';
                }
              }}
              className="w-10 h-10 object-contain aspect-square p-1 rounded-lg border border-slate-800/10 dark:border-slate-800/40 shadow-[0_0_12px_rgba(59,130,246,0.25)] dark:shadow-[0_0_16px_rgba(59,130,246,0.35)] overflow-visible" 
              style={{ imageRendering: '-webkit-optimize-contrast', objectFit: 'contain' }}
              alt="Toolix" 
            />
            <div className="flex flex-col justify-center">
              <span className="text-[25px] md:text-[26px] font-black tracking-tight font-display bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 dark:from-blue-400 dark:via-indigo-400 dark:to-purple-400 bg-clip-text text-transparent leading-none">
                Toolix
              </span>
              <span className="text-[9px] uppercase tracking-widest text-slate-500 font-mono font-bold mt-1">SaaS Platform</span>
            </div>
          </div>

          {/* MIDDLE DESKTOP NAVIGATION MENU (SEO and Professional layout) */}
          <div className="hidden md:flex items-center gap-6 text-[11px] font-extrabold uppercase tracking-wider text-slate-400 dark:text-slate-400">
            <button onClick={() => navigateTo('home')} className={`hover:text-blue-500 transition-colors cursor-pointer ${activeTab === 'home' ? 'text-blue-500 font-black' : ''}`}>
              {language === 'en' ? 'Home' : 'الرئيسية'}
            </button>
            <button onClick={() => navigateTo('tools')} className={`hover:text-blue-500 transition-colors cursor-pointer ${activeTab === 'tools' ? 'text-blue-500 font-black' : ''}`}>
              {language === 'en' ? 'Utilities' : 'الأدوات'}
            </button>
            <button onClick={() => navigateTo('support')} className={`hover:text-blue-500 transition-colors cursor-pointer ${activeTab === 'support' ? 'text-blue-500 font-black' : ''}`}>
              {language === 'en' ? 'Support' : 'الدعم'}
            </button>
            <button onClick={() => navigateTo('blog')} className={`hover:text-blue-500 transition-colors cursor-pointer ${['blog', 'blog-post'].includes(activeTab) ? 'text-blue-500 font-black' : ''}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 pb-28 relative z-10">
        ...
        {/* DYNAMIC BREADCRUMBS FOR CORPORATE / LEGAL / SYSTEM PAGES */}
        {!selectedToolId && (
          <Breadcrumbs 
            language={language}
            darkMode={darkMode}
          ...
          <div className="space-y-6">
            ...
            {activeTab === 'home' && (
              <div className="space-y-12 animate-fade-in">
                ...
                {/* ABOUT TOOLIX AI SECTION (Requirement 12, 16, 2500+ words target) */}
                <div className="space-y-6 border-t border-slate-800/15 pt-8" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                  <div className="text-center space-y-2 mb-6">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 text-[9px] font-black uppercase tracking-widest bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full">
                      <Info className="w-3 h-3 text-blue-400" />
                      <span>{language === 'en' ? 'Learn More About Us' : 'تعرّف على منصتنا ورسالتنا'}</span>
                    </span>
                    <h2 className="text-xl md:text-2xl font-black bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent">
                      {language === 'en' ? 'About Toolix AI Platform' : 'حول منصة توليكس الذكية (Toolix AI)'}
                    </h2>
                    <p className={`text-xs max-w-xl mx-auto leading-relaxed ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                      {language === 'en' 
                        ? 'Understand our secure browser-native architecture, our core philosophy, and why thousands of users trust our local tools daily.'
                        : 'اكتشف فلسفة المعالجة المحلية الآمنة، وهيكل منصتنا البرمجي المبتكر، وسبب ثقة آلاف المستخدمين اليومية في أدواتنا.'}
                    </p>
                  </div>

                  <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 text-xs leading-relaxed ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                    <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-slate-900/35 border-slate-850 hover:bg-slate-900/50 shadow-lg' : 'bg-white border-slate-200 hover:bg-slate-50 shadow-sm'} space-y-4`}>
                      <h3 className={`text-sm font-black ${darkMode ? 'text-white' : 'text-slate-900'} flex items-center gap-2`}>
                        <Cpu className="w-4 h-4 text-blue-400" />
                        <span>{language === 'en' ? 'Revolutionary Client-Side Architecture' : 'بنية برمجية محلية ثورية'}</span>
                      </h3>
                      <p>
                        {language === 'en'
                          ? 'Toolix AI is a premier, comprehensive suite of security-first, high-performance browser utilities engineered specifically to provide a modern, lightning-fast alternative to bloated, slow, and ad-ridden web utility websites. In the contemporary digital ecosystem, software developers, content writers, creative designers, financial planners, and students are constantly forced to upload sensitive documents, private parameters, and raw files to unknown remote servers just to perform basic everyday operations. Activities such as PDF merging, image file compression, cryptographic strong password generation, word counting, or JSON database structure formatting often require transferring critical records to backend databases. At Toolix AI, we believe your private data should stay exactly where it belongs: on your own device under your absolute personal custody.'
                          : 'تعتبر منصة توليكس الذكية (Toolix AI) بمثابة منصة برمجية متكاملة رائدة في تقديم مجموعة واسعة من الأدوات والخدمات المساعدة فائقة الأداء والمصممة خصيصاً لتوفير بديل عصري، آمن، وسريع للمواقع والأدوات التقليدية المليئة بالإعلانات والبرمجيات الخبيثة المبطنة. في العصر الرقمي الحالي، يضطر المطورون، وصناع المحتوى، والمهندسون، والطلاب لرفع مستنداتهم الحساسة وملفاتهم الخاصة إلى خوادم مجهولة المصدر لمجرد القيام بعمليات بسيطة واعتيادية مثل دمج ملفات PDF، ضغط الصور، توليد مفاتيح التشفير، أو تنسيق شيفرات JSON. في توليكس، نؤمن بقوة بأن بياناتك الشخصية يجب أن تظل في مكانها الصحيح والآمن: وهو جهازك الشخصي فقط دون أي وسيط خارجي قد يعرضها للتسريب أو التتبع.'}
                      </p>
                      <p>
                        {language === 'en'
                          ? 'Our entire application executes under a strict 100% client-side execution philosophy. By leveraging the latest advances in modern front-end browser technologies—such as standard HTML5 storage APIs, optimized client-side JavaScript execution, WebAssembly compilation (WASM) for heavy computations, and secure browser-native Web Crypto APIs—we have created an ecosystem where your inputs never touch the cloud. Every single byte of input data, uploaded assets, or configuration parameters is processed instantly in your local system memory (RAM). Absolutely zero documents, input strings, or uploaded elements are ever transmitted to our web servers, logged in remote databases, or exposed to third parties. This paradigm completely eliminates security hazards such as man-in-the-middle attacks, database server breaches, or cloud bucket leaks.'
                          : 'تعمل منصتنا بالاعتماد على فلسفة التشغيل المحلي الكامل (Client-Side) بنسبة 100%. من خلال استغلال أحدث تقنيات الويب المتطورة مثل لغة جافا سكريبت الحديثة، تقنيات المعالجة الفورية HTML5، محرّك الويب التجميعي WebAssembly (WASM) لمعالجة المهام الثقيلة، وتقنيات التشفير المدمجة في المتصفحات Web Crypto API، قمنا ببناء بيئة عمل رقمية متكاملة تقوم بمعالجة كافة البيانات والملفات والمدخلات مباشرة داخل الذاكرة العشوائية لجهازك (RAM). لا يتم نقل أي ملف، أو مستند، أو نص، أو صورة إلى خوادم خارجية على الإطلاق، كما لا يتم حفظ أي سجلات في قواعد بيانات سحابية. هذا النهج الصارم يحميك تماماً من ثغرات تسريب البيانات، هجمات الاختراق، وعمليات التتبع الإعلاني المزعجة.'}
                      </p>
                    </div>
                    ...
                    <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-slate-900/35 border-slate-850 hover:bg-slate-900/50 shadow-lg' : 'bg-white border-slate-200 hover:bg-slate-50 shadow-sm'} space-y-4`}>
                      <h3 className={`text-sm font-black ${darkMode ? 'text-white' : 'text-slate-900'} flex items-center gap-2`}>
                        ...
                        <span>{language === 'en' ? 'Ultimate Security & Target Audience' : 'أمان مطلق لجميع فئات المستخدمين'}</span>
                      </h3>
                      <p>
                        {language === 'en'
                          ? 'By processing files entirely locally, Toolix AI offers a robust haven for enterprise engineers debugging confidential JSON payloads, financial analysts structuring sensitive amortization coordinates, and medical professionals computing patient metrics. You do not need to sign up for subscriptions, register passwords, or pay premium quotas. Everything is available immediately in a clean glassmorphic desktop interface that can even be installed as a Progressive Web App (PWA) to run entirely offline on your laptop or smartphone. This makes it an invaluable utility suite for remote developers, mobile creators, and offline-focused digital professionals who cannot afford network latency or security risks.'
                          : 'من خلال معالجة الملفات محلياً بالكامل، توفر منصة توليكس ملاذاً برمجياً آمناً لمهندسي البرمجيات في كبرى الشركات لفحص شيفرات وقواعد بيانات العمل، والمحللين الماليين لتنظيم جداول حسابية غاية في الحساسية والأهمية، وصناع المحتوى الذين يتعاملون مع ملفات حقوق حصرية. لن تضطر أبداً للتسجيل أو إنشاء حساب أو استخدام بطاقات ائتمان. كل شيء متاح فوراً وبواجهة زجاجية تدعم التثبيت كتطبيق ويب مستقل (PWA) ليعمل بكفاءة مطلقة دون اتصال بالإنترنت في حاسبك الشخصي أو جوالك الذكي مما يجعله مثالياً للمطورين المتنقلين والمهنيين الملتزمين بالخصوصية.'}
                      </p>
                      <p>
                        {language === 'en'
                          ? 'To support our extensive free infrastructure and continue compiling high-performance tools, we partner with trusted digital advertising providers like Google AdSense. This integration serves targeted, compliant banner placements that power our development costs while strictly respecting user configuration. Furthermore, we aggregate anonymous traffic insights via Google Analytics 4 (GA4) to identify which tools are highly utilized, enabling us to concentrate our engineering effort on optimizing client-side performance, improving design metrics, and scaling our available tool library. Our cookies disclosures and data retention methodologies strictly conform to CCPA, GDPR, and Google Publisher Policies, assuring total transparency.'
                          : 'لدعم هذه البنية البرمجية الضخمة والاستمرار في بناء وتطوير المزيد من الأدوات المتقدمة، نتعاون مع شبكات إعلانية موثوقة مثل غوغل أدسنس لعرض إعلانات ممولة متوافقة بالكامل مع الشروط والقوانين الدولية. كما نقوم بمراقبة معدلات الاستخدام المجهولة وإحصاءات حركة التصفح عبر تحليلات غوغل (GA4) لمعرفة الأدوات الأكثر طلباً وتركيز جهودنا الهندسية على تطويرها وتحسين سرعة معالجتها لمتصفحك. كافة ملفات الكوكيز وسياسات حفظ البيانات تلتزم بالكامل بالمعايير العالمية لحماية البيانات والخصوصية مثل GDPR و CCPA لضمان تجربة تصفح آمنة وشفافة بالكامل.'}
                      </p>
                    </div>
                  </div>
                </div>

                {/* WHY CHOOSE TOOLIX AI (6 professional cards) */}
                <div className="space-y-6 border-t border-slate-800/15 pt-8" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                  <div className="text-center space-y-2 mb-6">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 text-[9px] font-black uppercase tracking-widest bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-full">
                      <Sparkles className="w-3 h-3 text-indigo-400" />
                      <span>{language === 'en' ? 'Key Advantages' : 'لماذا تختار منصتنا؟'}</span>
                    </span>
                    <h2 className="text-xl md:text-2xl font-black bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent">
                      {language === 'en' ? 'Why Choose Toolix AI?' : 'مزايا استثنائية تجعلنا الأفضل'}
                    </h2>
                    <p className={`text-xs max-w-xl mx-auto leading-relaxed ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                      {language === 'en' 
                        ? 'Compare our state-of-the-art secure local sandbox to legacy utilities and discover why we are the top choice.'
                        : 'قارن بيئة عملنا المحلية الآمنة بالمواقع والأدوات التقليدية لتكتشف مدى تميز وقوة منصة توليكس.'}
                    </p>
                  </div>
                    ...
                    {[
                      {
                        ...
                        titleEn: "100% Client-Side Processing",
                        titleAr: "معالجة محلية بالكامل 100%",
                        descEn: "Every transaction, conversion, compressor, and calculation operates strictly in browser memory (RAM). Absolute data security is guaranteed because your files and parameters never leave your local device.",
                        descAr: "تعمل كافة المنسقات والآلات الحاسبة ومضغطات الملفات مباشرة داخل ذاكرة متصفحك المؤقتة. لا يتم إرسال أو تخزين ملفاتك البرمجية أو مستنداتك الشخصية في أي ملقم خارجي."
                      },
                      {
                        icon: Laptop,
                        color: "bg-purple-500/10 text-purple-400",
                        titleEn: "SaaS-Grade PWA Offline Capabilities",
                        titleAr: "تشغيل مستقل دون إنترنت (PWA)",
                        descEn: "Fully configured as a Progressive Web App. Install Toolix directly on your Macbook, Windows computer, or smartphone with a single click and run extensive utilities offline anywhere.",
                        descAr: "تطبيق ويب تقدمي متطور قابل للتثبيت بضغطة زر واحدة على حاسوبك الشخصي أو هاتفك المحمول. قم بتشغيل المنصة وإنجاز المهام واستخدام الأدوات في أي مكان دون إنترنت."
                      },
                      {
                        icon: Zap,
                        color: "bg-amber-500/10 text-amber-400",
                        titleEn: "Sub-Millisecond Speed Performance",
                        titleAr: "سرعة استجابة مذهلة وصفرية",
                        descEn: "With zero backend network roundtrips, database server connections, or API delays, all algorithms, calculations, and compression workflows output results instantly and with maximum efficiency.",
                        descAr: "نظراً لعدم وجود اتصالات بخوادم خارجية أو معالجات سحابية، تتم عمليات الضغط والمعالجة والتشفير بشكل فوري وبزمن استجابة يقارب الصفر لمختلف أنواع المدخلات والملفات."
                      },
                      {
                        icon: Key,
                        color: "bg-indigo-500/10 text-indigo-400",
                        titleEn: "No Registrations or Paywall Limits",
                        titleAr: "مجانية حقيقية دون أي حسابات أو قيود",
                        descEn: "Experience fully unthrottled premium performance with zero account registrations, paywalls, or feature limits. Every single tool is instantly open to everyone, empowering developers and designers.",
                        descAr: "وصول غير محدود لجميع الخدمات والمميزات المعقدة مجاناً 100% ودون الحاجة لإنشاء حساب أو إدخال كلمات مرور أو تفعيل خطط شهرية. تمنح المنصة قوة تحكم مطلقة لجميع المبدعين."
                      },
                      {
                        icon: Lock,
                        color: "bg-emerald-500/10 text-emerald-400",
                        titleEn: "Global Compliance Laws Alignment",
                        titleAr: "التوافق مع القوانين الدولية وأدسنس",
                        descEn: "Our browser-native sandbox architecture fully aligns with global user rights protections, including CCPA, COPPA, and GDPR frameworks, guaranteeing a safe, compliant environment for search engines.",
                        descAr: "نهجنا البرمجي يتوافق تماماً مع التشريعات الدولية الخاصة بحماية سرية البيانات والخصوصية للأطفال والبالغين مما يضمن تجربة تصفح غاية في الأمان وتوافقاً مطلقاً مع محركات البحث."
                      },
                      {
                        icon: Globe,
                        color: "bg-red-500/10 text-red-400",
                        titleEn: "Sleek Glassmorphic Dual-Locale UX",
                        titleAr: "تصميم زجاجي فاخر باللغتين",
                        descEn: "A beautifully customized desktop layout styled with glassmorphic backgrounds and high-contrast styling. Full English and Arabic translations allow frictionless workspace configuration globally.",
                        descAr: "واجهة مستخدم زجاجية راقية غاية في الجمال والتناسق اللوني ومحسنة للتجاوب التام مع مختلف أحجام الشاشات. يتيح التبديل الفوري بين اللغتين بيئة عمل مريحة لجميع المحترفين."
                      }
                    ].map((card, idx) => (
                      <div
                        key={`why-${idx}`}
                      ...
                      >
                        <div className="space-y-4 text-left" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                          ...
                          <h3 className={`text-sm font-black ${darkMode ? 'text-white' : 'text-slate-900'} group-hover:text-blue-400 transition-colors`}>
                            {language === 'en' ? card.titleEn : card.titleAr}
                          </h3>
                          <p className="text-[11px] leading-relaxed text-slate-400">
                            {language === 'en' ? card.descEn : card.descAr}
                          </p>
                        </div>
                      </div>
                    ))}
                ...
                {/* HOW TOOLIX WORKS SECTION (Step-by-step instructions) */}
                <div className="space-y-6 border-t border-slate-800/15 pt-8" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                  <div className="text-center space-y-2 mb-6">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 text-[9px] font-black uppercase tracking-widest bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full">
                      <Settings className="w-3 h-3 text-blue-400" />
                      <span>{language === 'en' ? 'Platform Instructions' : 'خطوات بسيطة وسهلة'}</span>
                    </span>
                    <h2 className="text-xl md:text-2xl font-black bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent">
                      {language === 'en' ? 'How Toolix Works' : 'كيف تعمل منصة توليكس؟'}
                    </h2>
                    <p className={`text-xs max-w-xl mx-auto leading-relaxed ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                      {language === 'en' 
                        ? 'Follow these direct steps to securely process your development files, calculate statistics, or generate media assets.'
                        : 'اتبع هذه الخطوات البسيطة للبدء في معالجة ملفاتك، إجراء حساباتك، أو تشفير نصوصك بأمان تام.'}
                    </p>
                  </div>
                    ...
                    {[
                      {
                        ...
                        titleAr: "تصفح واختيار الأداة",
                        descEn: "Browse through our structured utility categories (Developer, Math, Text, Media, Business) or use the smart search bar to find your required tool instantly.",
                        descAr: "تصفح الفئات المختلفة للمنصة (المطورين، الرياضيات، الميديا، المستندات) أو استخدم محرك البحث الذكي للعثور على الأداة المطلوبة فوراً."
                      },
                      {
                        step: "02",
                        titleEn: "Configure Parameters",
                        titleAr: "إدخال الملفات والمعطيات",
                        descEn: "Paste your raw code payloads, drag and drop documents (such as PDF files), type input parameters, or adjust slider configuration options.",
                        descAr: "أدخل شفراتك البرمجية، أو ارفع مستنداتك المكتوبة (مثل ملفات PDF والصور)، أو أدخل الأرقام والمعاملات الحسابية في الحقول المخصصة."
                      },
                      {
                        step: "03",
                        titleEn: "Local RAM Execution",
                        titleAr: "معالجة محلية بالكامل",
                        descEn: "Our browser-native JavaScript and WebAssembly compiler scripts instantly process your metrics. No file data ever leaves your browser memory sandbox.",
                        descAr: "تقوم خوارزميات جافا سكريبت والمحركات المحلية في المتصفح بمعالجة مدخلاتك بشكل فوري وآمن داخل ذاكرة RAM دون إرسال أي ملف للخارج."
                      },
                      {
                        step: "04",
                        titleEn: "Export Your Output",
                        titleAr: "تصدير أو نسخ النتيجة بأمان",
                        descEn: "Instantly copy formatted JSON/YAML payloads, generated secure passwords, calculated graphs, or download compressed files directly with zero hassle.",
                        descAr: "بكبسة زر واحدة، يمكنك نسخ الأكواد المنسقة، كلمات المرور القوية المولدة، الجداول الحسابية، أو تحميل الملفات المطورة والمنجزة لجهازك بثقة."
                      }
                    ].map((step, idx) => (
                      <div
                        key={`step-${idx}`}
                        className={`p-5 rounded-2xl border flex flex-col justify-between transition-all duration-300 relative group ${darkMode ? 'bg-slate-900/35 border-slate-850 hover:bg-slate-900/50 shadow-lg' : 'bg-white border-slate-200 hover:bg-slate-50 shadow-sm'}`}
                      >
                        <div className="space-y-3 text-left" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                          <span className="text-2xl font-black bg-gradient-to-r from-blue-400 to-indigo-500 bg-clip-text text-transparent font-mono block">
                            {step.step}
                          </span>
                          <h3 className={`text-xs font-black ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                ...
                {/* SUPPORTED TOOLS CATEGORIES DEEP DIVE */}
                <div className="space-y-6 border-t border-slate-800/15 pt-8" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                  <div className="text-center space-y-2 mb-6">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 text-[9px] font-black uppercase tracking-widest bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full">
                      <LayoutDashboard className="w-3 h-3 text-blue-400" />
                      <span>{language === 'en' ? 'Ecosystem Capabilities' : 'بيئة عمل برمجية شاملة'}</span>
                    </span>
                    <h2 className="text-xl md:text-2xl font-black bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent">
                      {language === 'en' ? 'Our Comprehensive Tool Suite' : 'تنوع برمجي يلبي كافة احتياجاتك'}
                    </h2>
                    <p className={`text-xs max-w-xl mx-auto leading-relaxed ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                      {language === 'en' 
                        ? 'Explore our extensive library of highly optimized utilities designed for various industries and developer workflows.'
                        : 'اكتشف مجموعتنا الشاملة من الأدوات والبرمجيات المطورة خصيصاً لتوفير حلول فورية وآمنة لمختلف التخصصات.'}
                    </p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs leading-relaxed text-slate-400">
                    <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-slate-900/35 border-slate-850 hover:bg-slate-900/50' : 'bg-white border-slate-200'}`}>
                      <h3 className={`text-sm font-black ${darkMode ? 'text-white' : 'text-slate-900'} mb-3 flex items-center gap-2`}>
                        <Braces className="w-4 h-4 text-blue-400" />
                        <span>{language === 'en' ? 'Developer & Text Helpers' : 'أدوات المطورين ومعالجة النصوص'}</span>
                      </h3>
                      <p className="mb-3">
                        {language === 'en'
                          ? 'Our developer ecosystem provides robust client-side utilities including JSON Formatter & Validator, XML-to-YAML converter, Base64 encoder/decoder, and secure URL parser tools. Software engineers can safely paste proprietary payloads knowing no information is sent online.'
                          : 'توفر بيئتنا التقنية أدوات متكاملة للمطورين تشمل منسق ومدقق الـ JSON، محول صيغ XML إلى YAML، ومولدات التشفير وفك التشفير بصيغة Base64 ومحلل الروابط URL. يمكن للمبرمجين فحص Payload العمل بأمان تام ودون قلق من الاختراق.'}
                      </p>
                      <p>
                        {language === 'en'
                          ? 'We also support complex text utilities like high-performance word and character counters, responsive Markdown compilers, and text diff/merge checkers. These local scripts allow writers and publishers to evaluate documents securely in their browser memory sandbox.'
                          : 'ندعم أيضاً أدوات معالجة وتدقيق النصوص الفورية مثل عداد الكلمات والمحارف الدقيق، محررات ومجمعات نصوص ماركداون (Markdown)، وأداة فحص الفروقات بين النصوص. تساعد هذه البرمجيات الكُتّاب وصناع المحتوى على مراجعة مقالاتهم محلياً.'}
                      </p>
                    </div>
                    ...
                    <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-slate-900/35 border-slate-850 hover:bg-slate-900/50' : 'bg-white border-slate-200'}`}>
                      <h3 className={`text-sm font-black ${darkMode ? 'text-white' : 'text-slate-900'} mb-3 flex items-center gap-2`}>
                        ...
                        <span>{language === 'en' ? 'Math & Financial Calculators' : 'الرياضيات والحاسبات المالية'}</span>
                      </h3>
                      <p className="mb-3">
                        {language === 'en'
                          ? 'Our math module features premium algorithms including comprehensive loan amortization calculators, advanced compound interest engines, custom ratio generators, and percentage helpers. This allows individuals, business owners, and finance students to compute values without database logging.'
                          : 'يتميز قسم الرياضيات والحسابات المالية لدينا بخوارزميات غاية في الدقة لحساب استهلاك القروض (Amortization)، الفائدة المركبة التراكمية، ومولدات النسب المئوية المعقدة. تتيح هذه الأدوات للشركات والطلاب إجراء الحسابات دون الحاجة لقواعد بيانات خارجية تتبّع حساباتهم.'}
                      </p>
                      <p>
                        {language === 'en'
                          ? 'We also offer precise health metrics processing, including standard Body Mass Index (BMI) trackers and daily caloric maintenance processors. All computations are derived from rigorous mathematical equations (like the Mifflin-St Jeor formulas), giving user-friendly approximations.'
                          : 'نقدم أيضاً حاسبات متقدمة للقياسات الحيوية والصحية، مثل حاسبة مؤشر كتلة الجسم (BMI) وحاسبة السعرات الحرارية اليومية اللازمة للمحافظة على الوزن أو التخسيس بناءً على معادلات طبية ورياضية معتمدة دولياً (مثل Mifflin-St Jeor).'}
                      </p>
                    </div>
                    ...
                    <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-slate-900/35 border-slate-850 hover:bg-slate-900/50' : 'bg-white border-slate-200'}`}>
                      <h3 className={`text-sm font-black ${darkMode ? 'text-white' : 'text-slate-900'} mb-3 flex items-center gap-2`}>
                        ...
                        <span>{language === 'en' ? 'Media & Documents Converters' : 'الميديا وتحويل المستندات والملفات'}</span>
                      </h3>
                      <p className="mb-3">
                        {language === 'en'
                          ? 'Toolix AI hosts modern media processors, featuring an offline Image Compressor that operates purely in the browser Canvas, allowing you to reduce high-resolution JPEG, PNG, or WEBP assets. We also support vector-to-raster generators such as SVG-to-PNG compilers.'
                          : 'تضم توليكس برمجيات متطورة لمعالجة الصور والميديا والملفات محلياً 100% مثل مضغط الصور الفوري الذي يعمل على متصفحك مباشرة لتقليص أحجام صور JPEG و PNG و WEBP دون فقدان الجودة، ومحولات الرسوميات الشعاعية مثل محول SVG إلى PNG.'}
                      </p>
                      <p>
                        {language === 'en'
                          ? 'Additionally, our document tools integrate high-performance PDF merger and PDF page extractor libraries. By using standard JavaScript libraries running inside your local browser memory sandbox, we compile and split multi-page PDF documents without ever uploading the content to external networks.'
                          : 'علاوة على ذلك، توفر أدواتنا المستندية ميزة دمج ملفات PDF واستخراج الصفحات بكفاءة عالية وسرية تامة. يتم تنفيذ عمليات الدمج والتقسيم وتدوير الصفحات محلياً بالكامل من خلال حزم برمجية مخصصة تعمل في ذاكرة متصفحك دون إرسال المستندات إلى أي شبكة خارجية.'}
                      </p>
                    </div>
                  </div>
                </div>

                {/* PRIVACY & SECURITY SANDBOX POLICY DEEP DIVE */}
                <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-slate-900/35 border-slate-850 text-slate-300 shadow-lg' : 'bg-white border-slate-200 text-slate-600 shadow-sm'} space-y-4`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
                  <h3 className={`text-sm font-black ${darkMode ? 'text-white' : 'text-slate-900'} flex items-center gap-2`}>
                    <Lock className="w-4 h-4 text-blue-400" />
                    <span>{language === 'en' ? 'Our Privacy & Security Commitment' : 'ميثاق الأمان وحماية خصوصية بياناتك'}</span>
                  </h3>
                  <p className="text-xs leading-relaxed text-slate-400">
                    {language === 'en'
                      ? 'Because all tools on Toolix execute 100% locally in your browser memory (RAM), we do not maintain persistent database pipelines, file loggers, or data recovery systems for your inputs. In the event of an unexpected browser tab crash, device shutdown, or accidental cache clearance, your active workspace statistics may be permanently lost. You are solely responsible for keeping separate copies and backups of your private documents and code entries. We are fully committed to CCPA and GDPR compliance, ensuring complete transparent disclosures to AdSense partners and users alike. If you do not agree with any of our policies, please refrain from using our services. Your continued usage represents absolute consent.'
                      : 'بما أن جميع الأدوات في توليكس تعمل محلياً بنسبة 100% داخل متصفحك الشخصي (RAM)، فإننا لا نمتلك قواعد بيانات لحفظ أو استرجاع ملفاتك أو مخرجاتك. في حال حدوث عطل مفاجئ في متصفحك أو انقطاع التيار أو مسح الذاكرة العشوائية، قد تُفقد البيانات النشطة نهائياً. تقع على عاتقك بالكامل مسؤولية الاحتفاظ بنسخ احتياطية خاصة ومستقلة لملفاتك ومستنداتك. نحن نلتزم بالكامل بتشريعات حماية الخصوصية العالمية وسياستنا متوافقة تماماً مع شروط وإعلانات غوغل أدسنس. استمرارك في تصفح المنصة يمثل موافقة صريحة على كافة البنود والشروط المذكورة.'}
                  </p>
                </div>

                {/* FAQ ACCORDION SECTION (Requirement 6 - Expanded to 10+ real items) */}
                <div className="space-y-4 border-t border-slate-800/15 pt-8">
                  <div className="text-center space-y-2 mb-6" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 text-[9px] font-black uppercase tracking-widest bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full">
                      <HelpCircle className="w-3 h-3 text-blue-400" />
                      <span>{language === 'en' ? 'Help Center' : 'مركز المساعدة والاستفسارات'}</span>
                    </span>
                    ...
                    <p className="text-xs text-slate-400 max-w-lg mx-auto leading-relaxed">
                      {language === 'en' 
                        ? 'Everything you need to know about Toolix premium utility platform, user data processing, cookies disclosures, and privacy.'
                        : 'كل ما تحتاج لمعرفته حول منصة توليكس الاحترافية، أمن البيانات، سياسة الكوكيز، وكيفية المعالجة الآمنة.'}
                    </p>
                  </div>
                  ...
                  <div className="max-w-3xl mx-auto space-y-3.5" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                    {[
                      {
                        qEn: "Is Toolix really 100% free to use without any hidden fees?",
                        qAr: "هل منصة توليكس مجانية بالكامل حقاً ودون أي رسوم خفية؟",
                        ...
                        aAr: "نعم! منصة توليكس مجانية بالكامل دون الحاجة لتسجيل أي حساب، بريد إلكتروني، أو الاشتراك في خطط مدفوعة أو تجريبية. يمكنك استخدام جميع الأدوات الأساسية بشكل غير محدود وفوراً دون قيود مالية أو شروط مخفية."
                      },
                      {
                        qEn: "How does local browser processing protect my confidential data?",
                        qAr: "كيف تحمي ميزة المعالجة المحلية بياناتي وملفاتي الحساسة؟",
                        aEn: "Toolix is engineered with a strict 100% client-side architecture. This means all processing, calculations, PDF merging, text editing, and image compression take place solely in your browser's local memory (RAM). No files or text data are ever sent, logged, or cached on remote web servers.",
                        aAr: "تعتمد توليكس معيار أمان صارم يعمل محلياً بالكامل (Client-Side). جميع الملفات المرفوعة، كلمات المرور المولدة، والنصوص المكتوبة يتم معالجتها داخل ذاكرة متصفحك المؤقتة فقط، ولا يتم إرسالها أو حفظها نهائياً في ملقماتنا السحابية."
                      },
                      {
                        qEn: "Can I install Toolix on my device and use the utilities offline?",
                        qAr: "هل يمكنني تثبيت منصة توليكس على جوالي أو مكتبي لتشغيله دون إنترنت؟",
                        aEn: "Yes! Toolix supports full Progressive Web App (PWA) installation. Use your browser's built-in option (like 'Add to Home Screen' or the install icon in the URL bar) to instantly save Toolix to your device and run it offline without network latency.",
                        aAr: "بكل تأكيد! يدعم توليكس تقنية PWA بشكل متكامل. يمكنك استخدام ميزة المتصفح الافتراضية (مثل 'إضافة إلى الشاشة الرئيسية' أو أيقونة التثبيت في شريط العنوان) لحفظ تطبيق توليكس على جهازك واستخدامه دون اتصال بالإنترنت بكفاءة عالية."
                      },
                      {
                        qEn: "Does Toolix store cookies or track my online behavior?",
                        qAr: "هل يخزن موقع توليكس ملفات كوكيز أو يتتبع سلوك التصفح الخاص بي؟",
                        aEn: "Toolix uses cookies through trusted partners like Google AdSense to serve personalized advertisements and Google Analytics to collect aggregate, anonymous traffic statistics (such as bounce rate, geographic distribution, and page duration). Absolutely zero tool-supplied inputs are ever saved in cookies.",
                        aAr: "تستخدم منصة توليكس ملفات كوكيز آمنة وموثوقة من خلال شركائنا مثل غوغل أدسنس لخدمة الإعلانات وغوغل أناليتكس لجمع الإحصاءات العامة المجهولة (مثل مدة البقاء في الصفحة والموقع الجغرافي). لا يتم أبداً ربط أو تسجيل بيانات أدواتك بهذه الملفات."
                      },
                      {
                        qEn: "Why does the platform display Google AdSense advertisements?",
                        qAr: "لماذا تظهر إعلانات غوغل أدسنس على منصتكم؟",
                        aEn: "To cover server hosting costs, domain renewals, continuous software development, and infrastructure maintenance, we display Google AdSense ads. This monetization supports our mission to keep the entire library 100% free and open for everyone.",
                        aAr: "من أجل تغطية تكاليف الاستضافة، وتطوير الأدوات المستمر، وصيانة البنية التحتية، وتقديم كافة الأدوات بشكل مجاني، نقوم بعرض إعلانات غوغل أدسنس. يساعدنا هذا التمويل في إبقاء الخدمة مفتوحة ومتاحة للجميع مجاناً وبأعلى أداء."
                      },
                      {
                        qEn: "Are there any file size or conversion limits for PDFs and Images?",
                        qAr: "هل هناك حد أقصى لحجم ملفات PDF المرفوعة أو الصور المراد ضغطها؟",
                        aEn: "Since all file conversions, compression routines, and merging calculations execute on your local device RAM, the file size limit is strictly bounded by your browser memory and device hardware specifications. Toolix sets zero artificial upload caps.",
                        aAr: "بما أن كافة معالجات الصور والـ PDF والملفات البرمجية تتم محلياً بالكامل على متصفحك وجهازك الشخصي، فإن السعة المحددة تعتمد فقط على حجم ذاكرة جهازك وقدراته الهاردوير. لا تضع توليكس أي قيود أو حدود افتراضية."
                      },
                      {
                        qEn: "Is my source code secure when using compilers and formatting helpers?",
                        qAr: "هل كودي البرمجي آمن وسري تماماً عند استخدام منسقات ومجمعات الأكواد؟",
                        aEn: "Absolutely. When using the HTML, CSS, JavaScript, or Markdown compilers, your code is evaluated and rendered inside a sandboxed iframe in your local browser sandbox. Absolutely zero source code is transmitted to remote compilers, making it secure for company payloads.",
                        aAr: "بكل تأكيد. عند تشغيل المحررات والمجمعات البرمجية، يتم تشغيل ومعالجة الشيفرات البرمجية داخل إطار محمي ومعزول بالكامل في متصفحك الشخصي. لا يتم إرسال أي كود برمجي إلى معالجات خارجية، مما يجعله آمناً تماماً لأكواد الشركات والpayloads الحساسة."
                      },
                      {
                        qEn: "How often are new online tools and features added?",
                        qAr: "كم مرة يتم إضافة أدوات ومميزات برمجية جديدة إلى منصة توليكس؟",
                        ...
                        aAr: "يعمل فريقنا الهندسي باستمرار على بناء ونشر أدوات برمجية وحسابية ومستندية جديدة تلبي احتياجات المستخدمين! نقوم بتحديث المنصة بانتظام، ويمكنك إرسال اقتراحاتك لتطوير أدوات مخصصة عبر صفحة اتصل بنا."
                      },
                      {
                        qEn: "What are the core technology frameworks powering Toolix AI?",
                        qAr: "ما هي لغات وتقنيات البرمجة الأساسية المستخدمة لبناء توليكس؟",
                        aEn: "Toolix is engineered using React, Vite, Tailwind CSS, Lucide icons, and modern TypeScript. It leverages advanced APIs such as Web Crypto, HTML5 Canvas, and WebAssembly to achieve lightning-fast offline operations.",
                        aAr: "تم بناء منصة توليكس باستخدام أحدث التقنيات الاحترافية وتشمل ريأكت (React)، وتيلويند سي إس إس (Tailwind CSS)، وأيقونات لوسيد (Lucide)، وتايب سكريبت (TypeScript) مع الاعتماد على تقنيات Web Crypto و WebAssembly لتسريع العمليات."
                      },
                      {
                        qEn: "How can I submit feature requests, suggest tool ideas, or report bugs?",
                        qAr: "كيف يمكنني تقديم طلبات تطوير أدوات جديدة أو الإبلاغ عن مشكلات برمجية؟",
                        aEn: "We welcome your suggestions and bug reports! Navigate to our Contact page to fill out our official feedback form, or email our support desk directly at support@toolixappai.netlify.app. Our support team typically replies within 24-48 hours.",
                        aAr: "نرحب دائماً بكافة اقتراحاتكم وملاحظاتكم لتطوير المنصة! يمكنك الانتقال إلى صفحة 'اتصل بنا' لتعبئة نموذج التواصل الرسمي أو مراسلتنا مباشرة على البريد support@toolixappai.netlify.app حيث نجيب على كافة الرسائل في غضون 24-48 ساعة."
                      }
                    ].map((faq, index) => {
            ...
            {activeTab === 'contact' && (
              <div className="max-w-5xl mx-auto animate-fade-in text-left px-4" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                ...
                <div className="text-center space-y-3 max-w-2xl mx-auto mb-10">
                  ...
                  <p className="text-xs md:text-sm text-slate-400 leading-relaxed">
                    {language === 'en'
                      ? 'Have feedback, custom tool requests, or technical support inquiries? Reach out and our team will get back to you within 24-48 hours.'
                      : 'لديك اقتراح لأداة جديدة، أو تواجه مشكلة تقنية، أو تريد طلب شراكة؟ تواصل معنا فوراً وسيقوم فريقنا بالرد عليك ومساعدتك في غضون 24 إلى 48 ساعة.'}
                  </p>
                </div>
                    ...
                    <div className={`p-6 rounded-2xl border ${darkMode ? 'border-slate-800 bg-[#070b14]/50' : 'bg-slate-50 border-slate-200'} space-y-4`}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <a href="mailto:support@toolixappai.netlify.app" className="text-xs font-bold text-white hover:text-blue-400 transition-colors">
                            support@toolixappai.netlify.app
                          </a>
                        </div>
                      </div>
                    </div>
            ...
            {/* TAB CONTENT: COOKIE POLICY (Requirements 12, 16, AdSense Disclosure) */}
            {activeTab === 'cookie' && (
              <div className="max-w-4xl mx-auto space-y-6 text-left px-4 animate-fade-in text-xs leading-relaxed" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                <div className={`border-b pb-4 space-y-2 ${darkMode ? 'border-slate-800' : 'border-slate-200'}`}>
                  <h2 className={`text-2xl md:text-3xl font-black ${darkMode ? 'text-slate-100' : 'text-slate-900'}`}>
                    {language === 'en' ? 'Cookie Policy' : 'سياسة ملفات تعريف الارتباط والشفافية'}
                  </h2>
                  <p className="text-slate-500 font-bold">
                    {language === 'en' ? 'Last Updated: July 17, 2026' : 'آخر تحديث: 17 يوليو 2026'}
                  </p>
                </div>
                ...
                <div className="space-y-6 text-slate-400">
                  <p>
                    {language === 'en'
                      ? 'This Cookie Policy explains how Toolix ("we", "us", or "our") uses cookies and similar technologies to recognize you when you visit our website at toolix.app. It explains what these technologies are and why we use them, as well as your rights to control our use of them. This policy is written to comply fully with Google AdSense rules, GDPR, CCPA, and COPPA standards.'
                      : 'توضح هذه السياسة كيفية استخدام منصة توليكس (Toolix) لملفات تعريف الارتباط (Cookies) والتقنيات المشابهة عند زيارتك لموقعنا الإلكتروني عبر الرابط toolix.app. نوضح طبيعة هذه التقنيات، والسبب وراء استخدامها، وحقوقك الكاملة في التحكم بها وإدارتها بما يتوافق تماماً مع شروط غوغل أدسنس (Google AdSense) وقوانين حماية الخصوصية العالمية.'}
                  </p>
                  ...
                  <div className="space-y-2">
                    ...
                    <p>
                      {language === 'en'
                        ...
                        : 'ملفات تعريف الارتباط هي ملفات بيانات صغيرة يتم تخزينها على جهاز الكمبيوتر أو الهاتف المحمول الخاص بك عند تصفح موقع ويب معين. وتُستخدم هذه الملفات على نطاق واسع لتمكين المواقع من العمل بشكل سليم، وتوفير أداء أكثر سرعة وسلاسة، وتوليد تقارير إحصائية مجهولة المصدر.'}
                    </p>
                  </div>

                  <div className="space-y-2">
                    <h3 className={`text-sm font-black uppercase tracking-wider ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                      {language === 'en' ? '2. Why Do We Use Cookies?' : '2. لماذا نستخدم ملفات تعريف الارتباط؟'}
                    </h3>
                    <p>
                      {language === 'en'
                        ? 'Toolix uses first-party and third-party cookies for several reasons. Some cookies are required for technical reasons in order for our website to operate. Other cookies enable us to track and target the interests of our users to enhance the experience on our Online Platform. Third parties serve cookies through our website for advertising, analytics, and other purposes.'
                        : 'تستخدم منصة توليكس ملفات تعريف ارتباط خاصة بنا (طرف أول) وأخرى تابعة لجهات خارجية (طرف ثالث) لعدة أسباب. فبعض هذه الملفات ضروري من الناحية التقنية لتشغيل المنصة وإعدادات التفضيلات وحفظ المفضلة، بينما تساعدنا ملفات أخرى في تتبع اهتمامات مستخدمينا لتحسين تجربة التصفح وتقديم الخدمات. كما يقدم شركاؤنا ملفات لتخصيص الإعلانات وتحليل حركة المرور.'}
                    </p>
                  </div>

                  <div className="space-y-2">
                    <h3 className={`text-sm font-black uppercase tracking-wider ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                      {language === 'en' ? '3. Google AdSense & Personalized Advertising' : '3. إعلانات غوغل أدسنس والخصوصية الإعلانية'}
                    </h3>
                    <p>
                      {language === 'en'
                        ? 'Google, as a third-party vendor, uses cookies to serve advertisements on toolix.app. Google\'s use of the DART cookie enables it to serve ads to our users based on their visit to our site and other sites on the Internet. We incorporate personalized Google AdSense campaigns to fund our continuous free software operations. Users may opt out of personalized advertising by visiting Google Ads Settings (https://www.google.com/settings/ads) or by using industry-wide opt-out tools such as the Network Advertising Initiative.'
                        : 'تستخدم غوغل (Google)، بصفتها شريكاً إعلانياً خارجياً، ملفات تعريف ارتباط لتقديم الإعلانات على منصة توليكس. يتيح استخدام ملفات DART لغوغل عرض إعلانات مخصصة للمستخدمين بناءً على زياراتهم لموقعنا والمواقع الأخرى على شبكة الإنترنت. نحن ندمج إعلانات أدسنس لتوفير الموارد المالية وتقديم الأدوات مجاناً. يمكنك إلغاء تفعيل الإعلانات المخصصة عبر زيارة إعدادات إعلانات غوغل أو استخدام أدوات إلغاء الاشتراك المعتمدة دولياً.'}
                    </p>
                  </div>

                  <div className="space-y-2">
                    <h3 className={`text-sm font-black uppercase tracking-wider ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                      {language === 'en' ? '4. Google Analytics 4 (GA4) Disclosures' : '4. تتبع تحليلات جوجل (Google Analytics 4)'}
                    </h3>
                    <p>
                      {language === 'en'
                        ? 'This website integrates Google Analytics 4 (GA4) to collect anonymous web traffic statistics, page duration, bounce rates, and geographic location distribution. These cookies help us identify which tools are highly utilized and optimize server configurations and application response speeds. The data collected is fully aggregated, pseudonymized, and contains no personally identifiable or proprietary user information.'
                        : 'يدمج هذا الموقع تحليلات غوغل (Google Analytics 4) لجمع إحصاءات حركة المرور المجهولة، ووقت البقاء في الصفحات، وتوزيع الموقع الجغرافي للزوار. تساعدنا هذه الملفات في تحديد الأدوات الأكثر استخداماً لتركيز جهود التطوير عليها وتحسين سرعة استجابة المنصة. جميع البيانات مجمعة ومخفية الهوية ولا تحتوي على أي معلومات شخصية.'}
                    </p>
                  </div>

                  <div className="space-y-2">
                    <h3 className={`text-sm font-black uppercase tracking-wider ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                      {language === 'en' ? '5. Complete Integrity of Local Tool Execution' : '5. حماية خصوصية الأدوات المحلية بالكامل'}
                    </h3>
                    <p>
                      {language === 'en'
                        ? 'Please note: Although our third-party advertising and analytics partners set cookies to manage sessions and display metrics, the core utilities of Toolix (such as compilers, PDF tools, cryptographic generators, formatting scripts, and calculators) process your files and inputs 100% locally in your browser memory (RAM) sandbox. Absolutely zero user-supplied tool data, uploaded documents, or input parameters are processed by, saved in, or shared via tracking cookies.'
                        : 'ملاحظة هامة: على الرغم من أن شركاء الإعلانات والتحليلات يقومون بتعيين ملفات تعريف ارتباط لإدارة جلسات التصفح، فإن كافة الأدوات البرمجية الأساسية في توليكس (مثل منسقات النصوص، أدوات PDF، مولدات التشفير، والحاسبات) تعالج ملفاتك وبياناتك المدخلة محلياً 100% داخل ذاكرة متصفحك (RAM). لا يتم حفظ أو تتبع أو مشاركة أي من نصوصك أو ملفاتك عبر الكوكيز الإعلانية مطلقاً.'}
                    </p>
                  </div>

                  <div className="space-y-2">
                    <h3 className={`text-sm font-black uppercase tracking-wider ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                      {language === 'en' ? '6. How Can You Control Cookies?' : '6. كيف يمكنك التحكم في ملفات الكوكيز؟'}
                    </h3>
                    <p>
                      {language === 'en'
                        ? 'You have the absolute right to decide whether to accept or reject cookies. You can configure or amend your web browser controls to accept or refuse cookies. If you choose to reject cookies, you may still use our website with full access to all developer tools and calculators, although some minor preference settings (such as persisting dark mode) may reset on every visit. You can delete existing cookies directly from your browser settings at any time.'
                        : 'لديك الحق الكامل في قبول أو رفض ملفات تعريف الارتباط. يمكنك تهيئة متصفح الويب الخاص بك لحظر هذه الملفات أو تنبيهك عند استخدامها. في حال اختيارك رفض ملفات الكوكيز، سيظل بإمكانك استخدام موقعنا والوصول لكافة الأدوات والحاسبات مجاناً، غير أن بعض تفضيلات المظهر (مثل حفظ الوضع الداكن) قد يعاد تعيينها في كل زيارة جديدة.'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {/* TAB CONTENT: DISCLAIMER (Requirements 12, 16) */}
            {activeTab === 'disclaimer' && (
              <div className="max-w-4xl mx-auto space-y-6 text-left px-4 animate-fade-in text-xs leading-relaxed" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                <div className={`border-b pb-4 space-y-2 ${darkMode ? 'border-slate-800' : 'border-slate-200'}`}>
                  <h2 className={`text-2xl md:text-3xl font-black ${darkMode ? 'text-slate-100' : 'text-slate-900'}`}>
                    {language === 'en' ? 'Disclaimer' : 'إخلاء المسؤولية القانونية'}
                  </h2>
                  <p className="text-slate-500 font-bold">
                    {language === 'en' ? 'Last Updated: July 17, 2026' : 'آخر تحديث: 17 يوليو 2026'}
                  </p>
                ...
                <div className="space-y-6 text-slate-400">
                  <p>
                    {language === 'en'
                      ? 'The information and tools provided by Toolix on toolix.app are for general educational, informational, and utility purposes only. All utilities, calculators, and algorithms are provided on an "as-is" and "as-available" basis, without warranties of any kind, either express or implied.'
                      : 'إن المعلومات والأدوات البرمجية والحاسبة المقدمة من منصة توليكس عبر الرابط toolix.app هي لأغراض تعليمية وإرشادية وأدوات مساعدة عامة فقط. يتم توفير كافة الخدمات والحاسبات والبرمجيات النصية "كما هي" و "حسب توفرها"، دون تقديم أي ضمانات من أي نوع، سواء صريحة أو ضمنية.'}
                  </p>

                  <div className="space-y-2">
                    <h3 className={`text-sm font-black uppercase tracking-wider ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                      {language === 'en' ? '1. No Professional Advice' : '1. لا تمثل استشارات مهنية أو طبية أو مالية'}
                    </h3>
                    <p>
                      {language === 'en'
                        ? 'Toolix contains financial calculators (such as loan calculators), health calculators (such as Body Mass Index trackers), and technical scripts. The outputs generated by these tools represent mathematical approximations and predictions and do not constitute professional financial, tax, legal, medical, or dietary advice. Always consult a licensed professional before making substantial financial commitments, investments, or healthcare decisions.'
                        : 'تحتوي منصة توليكس على حاسبات مالية (مثل حاسبة القروض) وحاسبات صحية (مثل حاسبة مؤشر كتلة الجسم) وأدوات برمجية تقنية. إن النتائج والمخرجات الصادرة عن هذه الأدوات هي تقريبية ومبنية على معادلات رياضية قياسية، ولا يمكن اعتبارها بأي حال من الأحوال استشارات مالية أو ضريبية أو قانونية أو طبية مهنية. يرجى دائماً استشارة الخبراء المؤهلين قبل اتخاذ أي قرارات مالية أو صحية هامة.'}
                    </p>
                  </div>

                  <div className="space-y-2">
                    <h3 className={`text-sm font-black uppercase tracking-wider ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                      {language === 'en' ? '2. Accuracy of Mathematical Algorithms' : '2. دقة الخوارزميات والعمليات الحسابية'}
                    </h3>
                    <p>
                      {language === 'en'
                        ? 'While we utilize standard equations and rigorous programming guidelines to implement calculations and conversions, Toolix does not guarantee the complete accuracy, adequacy, validity, reliability, or completeness of any results generated. Any reliance you place on the output of calculators or text tools is strictly at your own risk. Toolix is not liable for mathematical errors, transient conversion glitches, or formatting discrepancies.'
                        : 'على الرغم من تطبيقنا لأقصى معايير التطوير والمعادلات الرياضية القياسية لضمان صحة ودقة عمل الأدوات، فإن منصة توليكس لا تضمن خلو النتائج تماماً من بعض الأخطاء التقنية أو البرمجية الطارئة. تقع مسؤولية الاعتماد على مخرجات هذه الحاسبات أو المنسقات على عاتق المستخدم وحده، ولا نتحمل أي مسؤولية عن أي خطأ حسابي أو تقني.'}
                    </p>
                  </div>

                  <div className="space-y-2">
                    <h3 className={`text-sm font-black uppercase tracking-wider ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                      {language === 'en' ? '3. Client-Side Integrity & User Responsibility' : '3. سلامة المعالجة المحلية ومسؤولية البيانات'}
                    </h3>
                    <p>
                      {language === 'en'
                        ? 'Because all tools on Toolix execute 100% locally in your browser memory (RAM), we do not have database pipelines, backups, or recovery solutions for your input files or processed data. In the event of a browser tab crash, device shutdown, or accidental cache clearance, your active work may be permanently lost. You are solely responsible for keeping separate copies and backups of your private files, documents, and code entries.'
                        : 'بما أن جميع الأدوات في توليكس تعمل محلياً بنسبة 100% داخل متصفحك الشخصي (RAM)، فإننا لا نمتلك قواعد بيانات لحفظ أو استرجاع ملفاتك أو مخرجاتك. في حال حدوث عطل مفاجئ في متصفحك أو انقطاع التيار أو مسح الذاكرة العشوائية، قد تُفقد البيانات النشطة نهائياً. تقع على عاتقك بالكامل مسؤولية الاحتفاظ بنسخ احتياطية خاصة ومستقلة لملفاتك ومستنداتك.'}
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    <h3 className={`text-sm font-black uppercase tracking-wider ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                      {language === 'en' ? '4. Third-Party Links & Partner Advertising' : '4. الروابط الخارجية والإعلانات الممولة'}
                    </h3>
                    <p>
                      {language === 'en'
                        ? 'Our platform integrates Google AdSense and may contain links to external web resources. These links are served dynamically by ad networks or partnerships. Toolix does not warrant, endorse, guarantee, or assume responsibility for the accuracy or reliability of any information offered by third-party websites linked through our platform. Accessing any external links is completely at your own discretion and risk.'
                        : 'تدمج منصتنا إعلانات غوغل أدسنس (Google AdSense) لتأمين تكاليف التشغيل المجاني، وقد تحتوي على روابط تشير إلى مواقع خارجية. لا تملك توليكس أي سيطرة على محتوى تلك الروابط أو دقتها أو أمانها، ولا ندعم أو نتبنى أي منتجات أو خدمات معلن عنها خارج نطاق موقعنا. تصفحك للروابط الخارجية يقع على مسؤوليتك الخاصة.'}
                    </p>
                  </div>

                  <div className="space-y-2">
                    <h3 className={`text-sm font-black uppercase tracking-wider ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                      {language === 'en' ? '5. Exclusion of Liability' : '5. حصر المسؤولية القانونية'}
                    </h3>
                    <p>
                      {language === 'en'
                        ? 'To the maximum extent permitted by applicable law, in no event shall Toolix, its creators, or its contributors be liable for any special, incidental, indirect, consequential, or punitive damages whatsoever (including, without limitation, damages for loss of business profits, business interruption, loss of program data, or any other pecuniary loss) arising out of the use or performance of this website and its utility tools.'
                        : 'بأقصى حد يسمح به القانون المعمول به، لا نتحمل نحن أو مطورو الموقع بأي حال من الأحوال المسؤولية عن أي أضرار خاصة أو عرضية أو غير مباشرة أو تبعية أو جزائية من أي نوع (بما في ذلك على سبيل المثال لا الحصر، أضرار خسارة الأرباح، توقف الأعمال، فقدان البيانات، أو أي خسائر مالية أخرى) تنتج عن استخدام أو عدم القدرة على استخدام خدماتنا.'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {/* TAB CONTENT: CORPORATE COMPANY PAGES */}
            {['about', 'mission', 'vision', 'team', 'story', 'support'].includes(activeTab) && (
              <CompanyPages 
                activeTab={activeTab}
                language={language}
                darkMode={darkMode}
            ...
            {/* TAB CONTENT: BLOG PAGES */}
            {['blog', 'blog-post'].includes(activeTab) && (
              <Blog 
                language={language}
                darkMode={darkMode}
      ...
      <footer className={`border-t py-12 transition-colors duration-300 ${darkMode ? 'bg-[#010309] border-slate-900 text-slate-400' : 'bg-slate-100 border-slate-200 text-slate-600'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-1 md:grid-cols-4 gap-8 text-left" dir={language === 'ar' ? 'rtl' : 'ltr'}>
          {/* Logo & Slogan Column */}
          <div className="space-y-3.5">
            <div className="flex items-center gap-3">
              <img 
                ...
                onError={(e) => {
                  const img = e.currentTarget;
                  if (img.src.endsWith('/assets/logo.png')) {
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col gap-2 text-[10px] font-bold">
              <button onClick={() => navigateTo('home')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Dashboard Home' : 'الرئيسية'}</button>
              <button onClick={() => navigateTo('tools')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Browse All Utilities' : 'تصفح كافة الأدوات'}</button>
              <button onClick={() => navigateTo('favorites')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Saved Favorites' : 'الأدوات المفضلة'}</button>
              <button onClick={() => navigateTo('settings')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Platform Settings' : 'إعدادات المنصة'}</button>
              <button onClick={() => navigateTo('sitemap')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'HTML Sitemap' : 'خريطة الموقع (Sitemap)'}</button>
              <button onClick={() => navigateTo('status')} className="hover:text-blue-400 text-left transition-colors text-emerald-400 flex items-center gap-1.5 cursor-pointer">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-extrabold uppercase tracking-wider text-slate-200">{language === 'en' ? 'Corporate' : 'الشركة والمصادر'}</h4>
            <div className="flex flex-col gap-2 text-[10px] font-bold">
              <button onClick={() => navigateTo('about')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'About Us' : 'من نحن وعن رسالتنا'}</button>
              <button onClick={() => navigateTo('mission')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Our Mission' : 'رسالتنا وأهدافنا'}</button>
              <button onClick={() => navigateTo('vision')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Our Vision' : 'رؤية المنصة'}</button>
              <button onClick={() => navigateTo('team')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Meet The Team' : 'فريق العمل والخبراء'}</button>
              <button onClick={() => navigateTo('story')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Our Story' : 'قصة تأسيس توليكس'}</button>
              <button onClick={() => navigateTo('support')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Support Center' : 'مركز الدعم والمساعدة'}</button>
              <button onClick={() => navigateTo('blog')} className="hover:text-blue-400 text-left transition-colors text-blue-400 flex items-center gap-1 cursor-pointer">
                <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />
                <span>{language === 'en' ? 'Knowledge Blog' : 'المدونة ومقالات المعرفة'}</span>
              </button>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col gap-2 text-[10px] font-bold">
              <button onClick={() => navigateTo('privacy')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Privacy Policy' : 'سياسة الخصوصية وأمن البيانات'}</button>
              <button onClick={() => navigateTo('terms')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Terms of Service' : 'شروط الخدمة والاستخدام'}</button>
              <button onClick={() => navigateTo('cookie')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Cookie Policy' : 'سياسة ملفات تعريف الارتباط'}</button>
              <button onClick={() => navigateTo('disclaimer')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Disclaimer' : 'إخلاء المسؤولية القانونية'}</button>
              <button onClick={() => navigateTo('contact')} className="hover:text-blue-400 text-left transition-colors cursor-pointer">{language === 'en' ? 'Contact Support' : 'اتصل بنا والدعم الفني'}</button>
            </div>
          </div>
        </div>
      </footer>
